...
import { Chip, Divider, IconButton, Menu, Searchbar, Text } from 'react-native-paper';
import { useSafeAreaInsets } from 'react-native-safe-area-context';

import { OpportunityCard } from '@/src/components/OpportunityCard';
import { opportunities, type OpportunityCategory } from '@/src/data/opportunities';
import { usePreferences } from '@/src/contexts/preferences-context';

const CATEGORY_FILTERS = ['All', 'Scholarships', 'Fellowships', 'Exchange', 'Internships', 'Grants', 'Competitions'] as const;
  ...
  return (
    <ScrollView
      ...
      <ScrollView
        ...
        {CATEGORY_FILTERS.map((filter) => (
          <Chip
            ...
            style={category === filter ? { backgroundColor: paperTheme.colors.primary } : undefined}
            textStyle={category === filter ? { color: paperTheme.colors.onPrimary } : undefined}>
            {filter}
          </Chip>
        ))}
      </ScrollView>
      ...
      <View style={styles.resultsHeader}>
        <Text variant="titleMedium" style={{ color: paperTheme.colors.onSurface, fontWeight: '700' }}>
      ...
      {filtered.length === 0 ? (
        <View style={styles.empty}>
          ...
          <Text variant="bodySmall" style={[styles.emptySubtext, { color: paperTheme.colors.onSurfaceVariant }]}>
